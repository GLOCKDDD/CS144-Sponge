...
class NetworkInterface {
  private:
    ...
    //! IP 地址 映射 MAC 地址 + 剩余有效时间
    std::unordered_map<uint32_t, ARPEntry> _arp_cache{};

    //! IP 地址 映射 距离允许再次发 ARP request 还剩多久（5秒内不能重复发同一arp request）
    std::unordered_map<uint32_t, size_t> _pending_arp_requests{};

    //! next_hop IP 映射 一批等待发送的 InternetDatagram（如果没有ip->到mac，先排队）
    std::unordered_map<uint32_t, std::list<InternetDatagram>> _waiting_datagrams{};

    static constexpr size_t ARP_CACHE_TIMEOUT = 30000;
    static constexpr size_t ARP_REQUEST_TIMEOUT = 5000;

    void send_frame(const EthernetAddress &dst, const uint16_t type, BufferList payload);
    void learn_mapping(const uint32_t ip_address, const EthernetAddress &ethernet_address);
